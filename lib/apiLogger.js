 ...
 */
export function logApiCall(method, endpoint, statusCode, loadTime = 0, additionalData = {}) {
  const logMessage = `${method} ${endpoint} - ${statusCode} - ${loadTime}ms`

  logActivity('api_request', statusCode >= 400 ? 'error' : 'info', logMessage, {
    method,
    url: endpoint,
    loadTime,
    statusCode,
...
export function createApiLogger(req, action = 'api_call', additionalData = {}) {
  ...
  return (statusCode, error = null) => {
    ...
    logApiCall(method, action, statusCode, loadTime, {
      userAgent,
      ip,
