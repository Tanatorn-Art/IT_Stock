...
export async function createBackup(reason = 'manual') {
  try {
    ...
    if (fs.existsSync(dbPath)) {
      ...
      await logActivity('backup', 'success', `Created backup: ${backupFileName}`, { reason, fileName: backupFileName })
    ...
    await logActivity('backup', 'error', `Backup failed: ${error.message}`, { reason })
    throw error
  }
}
...
export async function restoreFromBackup(backupFileName) {
  try {
    ...
    await logActivity('restore', 'success', `Restored from backup: ${backupFileName}`, { fileName: backupFileName })
    
    return true
  } catch (error) {
    console.error('Restore failed:', error)
    await logActivity('restore', 'error', `Restore failed: ${error.message}`, { fileName: backupFileName })
    throw error
  }
}
...
export async function deleteBackup(backupFileName) {
  try {
    ...
    if (fs.existsSync(backupPath)) {
      ...
      await logActivity('backup_delete', 'success', `Deleted backup: ${backupFileName}`, { fileName: backupFileName })
      return true
    } else {
      throw new Error('Backup file not found')
    }
    ...
    await logActivity('backup_delete', 'error', `Failed to delete backup: ${error.message}`, { fileName: backupFileName })
    throw error
  }
}
...
export async function cleanupOldBackups(keepCount = 10) {
  try {
    ...
    if (backups.length > keepCount) {
      ...
      await logActivity('backup_cleanup', 'success', `Cleaned up ${toDelete.length} old backups`, { deletedCount: toDelete.length })
    }
  } catch (error) {
    console.error('Backup cleanup failed:', error)
    await logActivity('backup_cleanup', 'error', `Backup cleanup failed: ${error.message}`, {})
  }
}
