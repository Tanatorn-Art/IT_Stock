...
// Update image paths to use simple API route format
async function updateToSimpleApi() {
  const db = getDatabase();

  console.log('Updating to simple API route format...');
  console.log('===================================');
  ...
  for (const item of stockWithImages) {
    ...
    // Convert /api/images/filename.webp to /api/images?filename=filename.webp
    if (currentPath.startsWith('/api/images/') && !currentPath.includes('?filename=')) {
...
// Run the function
updateToSimpleApi().catch(console.error);
